                   ...
                   void *cbData)
{
    ...
    if (service == nullptr) {
        CCEC_LOG(LOG_ERR, "AidlHAL::open failed: IHdmiCec service unavailable\r\n");
    ...
    if (!s.isOk() || controller == nullptr) {
        CCEC_LOG(LOG_ERR, "AidlHAL::open failed: service->open status not OK or controller is null\r\n");
        return HDMI_CEC_IO_GENERAL_ERROR;
    }
